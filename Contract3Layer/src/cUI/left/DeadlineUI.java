...
public class DeadlineUI extends JDialog implements ActionListener{
	JLabel label1,label2;
	JTextField textField1,textField2;
	JButton button1,button2;
	JPanel panel1,panel2,panel3,panel4;
	JTable table;
	public DeadlineUI(Frame fck,String ckm,Boolean msck){
		super(fck,ckm,msck);
		
		label1=new JLabel("请设置到期提醒的天数：（单位：天）");
		label1.setFont(MyFont.f0);
		label2=new JLabel("到期提醒的天数：");
		label1.setFont(MyFont.f3);
		
		textField1=new JTextField(5);
		textField1.setText("14");
	...
	public JTable deadlineAlarm(){
		table=new JTable();
		String sql="select * from Contract where DATEDIFF(DD,getdate(),CDeadline)<=?";
